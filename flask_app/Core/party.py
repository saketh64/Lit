...
PUBLIC METHODS:
    ...
    downvote_song(user_id, song_url)
    get_queue_json(user_id)
...
class Party:
    ...
    def add_song(self, user_id, song_url, title):
        if any(song.url == song_url for song in self.queue):
            ...
            self.queue.append(
                Song(title, song_url, user_id)
            )
            self.reorder_queue()


    def upvote_song(self, user_id, song_url):
        ...
        # avoid double voting
        if user_id in song.upvotes:
            print "User tried to upvote song '%s' again - ignoring." % song.title
            return

        song.upvotes.append(user_id)
        song.downvotes.remove(user_id) # remove user's downvote if there

        self.reorder_queue()


    def downvote_song(self, user_id, song_url):
        song = next((song for song in self.queue if song.url == song_url), None)
        ...
        # avoid double voting
        if user_id in song.downvotes:
            print "User tried to upvote song '%s' again - ignoring." % song.title
            return

        song.downvotes.append(user_id)
        song.upvotes.remove(user_id) # remove user's downvote if there

        self.reorder_queue()


    def get_queue_json(self, user_id):
        # user is used to give the user THEIR particular vote data
        return [song.get_json(user_id) for song in self.queue]
